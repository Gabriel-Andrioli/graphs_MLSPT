...
public:
    ...
    GrafoWrapper(bool orientado, bool ponderado)
    {
        grafoReal = new Graph(orientado, ponderado);
    }
    ...
    void inserirVertice(int v) { grafoReal->add_vertex(v); }
    void inserirAresta(int u, int v, double peso = 1.0) { grafoReal->add_edge(u, v, peso); }
    bool verificarAresta(int u, int v) { return grafoReal->has_edge(u, v); }
    void alterarPesoAresta(int u, int v, double peso) { grafoReal->set_edge_weight(u, v, peso); }
    int grauVertice(int v) { return grafoReal->degree(v); }
    std::vector<int> listarVizinhos(int v) { return grafoReal->get_neighbors(v); }
    bool saoAdjacentes(int u, int v) { return grafoReal->are_adjacent(u, v); }
    void removerAresta(int u, int v) { grafoReal->remove_edge(u, v); }
    void removerVertice(int v) { grafoReal->remove_vertex(v); }
...
void testAll()
{
    ...
    GrafoWrapper gNaoOrientado(false, true);
    GrafoWrapper gOrientado(true, true);
